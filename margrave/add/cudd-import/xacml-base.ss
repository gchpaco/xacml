(module xacml-base mzscheme
  ...
  (require "xacml-cudd.scm"
           "../../lib/file-utils.scm")
  
  (provide xacml-print
           xacml-change-print
           xacml-change-only-print
           restrict-to
           ;restrict-to-permit
           ;restrict-to-deny
           always-false-vars
           ...
           DE_VAL
           EE_VAL
           (all-from "xacml-cudd.scm"))
  
  
  
  ;; These must be in sync with the values found in the C code.
  ...
  ;; skip 10 and 11 because 
  ;; look like binary strings
  ...
  (define (xacml-print/table add mgr table-func)
    (run-with-temp-file 
     (lambda (temp-file-name)
       (print-minterms mgr add temp-file-name)
  ...
  (define (xacml-change-print mgr add)
  ...
  (define (xacml-change-only-print mgr add)
    (run-with-temp-file
     (lambda (temp-file-name)
       (print-minterms mgr add temp-file-name)
       (with-input-from-file temp-file-name
  ...
  (define (restrict-to mgr add const)
    (cudd-add-restrict-interval mgr add const const))
  
  ;; restrict-to-permit : ADD manager -> 0-1ADD
  ;; returns 0-1ADD of all permit cases from original ADD
  (define (restrict-to-permit mgr add)
    (restrict-to mgr add PERMIT_VAL))
  
  ;; restrict-to-deny : ADD manager -> 0-1ADD
  ;; returns 0-1ADD of all deny cases from original ADD
  (define (restrict-to-deny mgr add)
    (restrict-to mgr add DENY_VAL))
