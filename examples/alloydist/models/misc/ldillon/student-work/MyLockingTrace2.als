...
static sig MutexOrder{
  ...
  firstmutex: Mutex,
  lastmutex: Mutex,
  nextmutex: (allmutex-lastmutex) !->! (allmutex-firstmutex)
}{
  // mutex is totally ordered by *nextmutex and firstmutex is the first mutex
  firstmutex.*nextmutex = allmutex
}
...
det fun GrabMutex( s:State, p:Process, m:Mutex ):State {
  ...
  // the mutex to acquire should have higher order than those the process already acuqired
  all mo: MutexOrder | all mu in p.s::holds | m in mu.*(mo.nextmutex)
