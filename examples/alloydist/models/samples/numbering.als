...
fun AddStyleToContext (s, s': State, style: Style) {
...
fun AddStyleToNumbering (s, s': State, style: Style) {
	s'.value[style] = 
		if style in s.context then s.value[style].next else style.initial
	s'.context = s.context - style.replaces + style
	all x: Style - style |
		s'.value[x] = 
...
fun AddStyle (s, s': State, style: Style) {
