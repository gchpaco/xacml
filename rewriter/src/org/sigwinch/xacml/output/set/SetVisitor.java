...
public class SetVisitor extends CodeVisitor {
    ...
    public String getFunctions () { return functions.toString (); }
    ...
    /**
     * Given a set representation, TRUE represents the entire
       environmental set, and FALSE represents the empty set.
     *
	...
	fa.print (maybeWrap (solePredicate.getSet (), 
    ...
    /**
     * Write code for anded predicates; this is just set intersection
       for us.
     *
	...
	map.setTextFor (andPredicate,
			binaryFunction ("&", andPredicate.getLeft (),
					andPredicate.getRight (), andPredicate,
					PrecedencePair.INTERSECT),
			PrecedencePair.INTERSECT);
    }

    /**
     * Write code for ored predicates; this happens to be set union
	...
	map.setTextFor (orPredicate,
			binaryFunction ("+", orPredicate.getLeft (),
					orPredicate.getRight (), orPredicate,
					PrecedencePair.UNION),
			PrecedencePair.UNION);
    }

    /**
     * Write code for environmental predicates; this is just a
     * variable reference, really.
     *
    ...
    /**
     * Write code for constant value predicates; this is again just a
     * variable reference.
     *
	...
	public void visitEquality (Predicate first, Predicate second) {
	    ...
	    fa.print (expr);
	    fa.print (" extends E {} { ");
	    fa.print (binaryFunction ("in", predicate.getAttribute (), 
				      predicate.getBag (), PrecedencePair.EQ));
	    fa.println (" }");
	...
	public void visitDefault (String string, Predicate [] arguments) {
	    ...
	    printFunction (functionName, "E", "Bool",
			   "xpathnodematch (" +
	...
	public void visitAnd (Predicate [] arguments) {
	    ...
	    text.append (maybeWrap (arguments[0], func,
				    PrecedencePair.INTERSECT));
