...
public class StaticVisitor extends VisitorImpl {
    ...
    public void incrementType(String name) {
        ...
        map.put(name, new Integer(map.get(name).intValue() + 1));
    }

    @Override
    public void walkConstantValuePredicate(ConstantValuePredicate predicate) {
    ...
    public void walkFunctionCallPredicate(
        ...
        if (alreadySeen.contains(new Integer(functionCallPredicate.getIndex())))
            return;
        alreadySeen.add(new Integer(functionCallPredicate.getIndex()));
        StaticFunctionVisitor v = new StaticFunctionVisitor(this);
