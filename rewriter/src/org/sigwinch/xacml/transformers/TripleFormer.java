...
public class TripleFormer extends TransformerImpl {
    ...
    public Tree walkScope(Scope scope) {
        ...
        if (!(scope.getChild() == Permit.PERMIT || scope.getChild() == Deny.DENY))
        ...
        return new Triple(triple.getPermit().andWith(predicate), triple
                .getDeny().andWith(predicate), triple.getError().andWith(
                predicate));
    }
    ...
    public Tree walkError(Error error) {
        ...
        return new Triple(triple.getPermit().andWith(not), triple.getDeny()
                .andWith(not), triple.getError().orWith(predicate));
    }
