...
public class TriplePropagator extends TransformerImpl {
	...
	return new Triple (s1.orWith (s2).andWith (t1.orWith (t2).not ()),
			   (r1.andWith (s2.orWith (t2).not ()))
			   .orWith (r2.andWith (s1.orWith (t1).not ())),
			   t1.orWith (t2));
    }
	...
	Predicate t1 = left.getError ();
	Predicate t2 = right.getError ();
	return new Triple ((s1.andWith (r2.orWith (t2).not ()))
			   .orWith (s2.andWith (r1.orWith (t1).not ())),
			   r1.orWith (r2).andWith (t1.orWith (t2).not ()),
			   t1.orWith (t2));
    }
	...
	Predicate t1 = left.getError ();
	Predicate t2 = right.getError ();
	return new Triple (s1.orWith (s2).andWith (s1.andWith (s2).orWith (t1)
						   .orWith (t2).not ()),
			   r1.orWith (r2).andWith (r1.andWith (r2).orWith (t1)
						   .orWith (t2).not ()),
			   t1.orWith (t2).orWith (s1.andWith (s2))
			   .orWith (r1.andWith (r2)));
    }
	...
	Predicate t1 = left.getError ();
	Predicate t2 = right.getError ();
	return new Triple (s1.orWith (s2.andWith (r1.orWith (t1).not ())),
			   r1.orWith (r2.andWith (s1.orWith (t1).not ())),
			   t1.orWith (t2.andWith (s1.orWith (r1).not ())));
    }
