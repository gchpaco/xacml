...
public class PolicySetParser extends AbstractParser {
    ...
    /**
     * Reads the given <code>PolicySet</code> node and creates and
     * returns an appropriate <code>Tree</code>.  This reads the
     * PolicyCombiningAlgId attribute.
     *
     * @param element an <code>Element</code> representing a <code>PolicySet</code> node
	...
	for (int i = subrules.getLength () - 2; i >= 0; i--) {
	    ...
	    base = ruleToTree (getXACMLAttribute (element, "PolicyCombiningAlgId"),
			       leaf, base);
	}
