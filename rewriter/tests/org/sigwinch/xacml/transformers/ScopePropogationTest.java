...
public class ScopePropogationTest extends TestCase {
	...
	Tree expectedResult = 
	    new Error (new Scope (Deny.DENY, a),
		       new AndPredicate (b, a));
	assertEquals (expectedResult,
		      tree.transform (new Propagator ()));
    }

    public void testThroughPermit () {
	Tree tree = new Scope (new PermitOverridesRule (Deny.DENY,
							Permit.PERMIT),
			       a);
	Tree expectedResult =
	    new PermitOverridesRule (new Scope (Deny.DENY, a),
				     new Scope (Permit.PERMIT, a));
	assertEquals (expectedResult, tree.transform (new Propagator ()));
    }

    public void testThroughDeny () {
	Tree tree = new Scope (new DenyOverridesRule (Deny.DENY,
						      Permit.PERMIT),
			       a);
	Tree expectedResult =
	    new DenyOverridesRule (new Scope (Deny.DENY, a),
				   new Scope (Permit.PERMIT, a));
	assertEquals (expectedResult, tree.transform (new Propagator ()));
    }

    public void testThroughFirst () {
	Tree tree = new Scope (new FirstApplicableRule (Deny.DENY,
							Permit.PERMIT), a);
	Tree expectedResult =
	    new FirstApplicableRule (new Scope (Deny.DENY, a),
				     new Scope (Permit.PERMIT, a));
	assertEquals (expectedResult, tree.transform (new Propagator ()));
    }
	...
	Tree expectedResult =
	    new OnlyOneRule (new Scope (Deny.DENY, a),
			     new Scope (Permit.PERMIT, a));
	assertEquals (expectedResult, tree.transform (new Propagator ()));
    }
	...
	Tree tree = new OnlyOneRule (new Scope (new PermitOverridesRule 
				     ...
				     Deny.DENY);
	Tree expectedResult =
	    new OnlyOneRule (new PermitOverridesRule 
			     (new Scope (Deny.DENY, a),
			      new Scope (Permit.PERMIT, a)), Deny.DENY);
	assertEquals (expectedResult, tree.transform (new Propagator ()));
    }
	...
	Tree tree = 
	    new Scope (new DenyOverridesRule 
		       (new Scope (new Error (new Scope (new Scope 
							 (Permit.PERMIT, p),
							 p), p), p),
			new Scope
			(new DenyOverridesRule
			 (new Error (new Scope (new Scope 
						(Permit.PERMIT, p),
						p), p),
			  new DenyOverridesRule
			  (new Error (new Scope (new Scope
						 (Permit.PERMIT, p),
						 p), p),
			   new Scope (Deny.DENY, p))),
			 p)), p);
	Tree expectedResult = 
	    new DenyOverridesRule 
	    (new Error (new Scope (Permit.PERMIT, p), p),
	     new DenyOverridesRule
	      (new Error (new Scope (Permit.PERMIT, p), p),
	       new DenyOverridesRule
	       (new Error (new Scope (Permit.PERMIT, p), p),
