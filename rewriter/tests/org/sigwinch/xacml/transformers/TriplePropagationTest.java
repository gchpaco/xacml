...
public class TriplePropagationTest extends TestCase {
    ...
    public void testThroughOne() {
        Tree tree = new OnlyOneRule(new Triple(a, b, c), new Triple(d, e, f));
        Tree expectedResult = new Triple(a.orWith(d).andWith(
                a.andWith(d).orWith(c).orWith(f).not()), b.orWith(e).andWith(
                b.andWith(e).orWith(c).orWith(f).not()), c.orWith(f).orWith(
                a.andWith(d)).orWith(b.andWith(e)));
        assertEquals(expectedResult, tree.transform(new TriplePropagator()));
    }

    public void testThroughFirst() {
        Tree tree = new FirstApplicableRule(new Triple(a, b, c), new Triple(d,
                e, f));
        Tree expectedResult = new Triple(
                a.orWith(d.andWith(b.orWith(c).not())), b.orWith(e.andWith(a
                        .orWith(c).not())), c.orWith(f.andWith(a.orWith(b)
                        .not())));
        assertEquals(expectedResult, tree.transform(new TriplePropagator()));
    }
    ...
    public void testWithNegatives() {
        ...
        Tree tree = new DenyOverridesRule(new Triple(a, nil, b), new Triple(
                nil, c, nil));
        /*
    ...
    public void testElementaryOperations() {
        ...
        assertEquals(a.andWith(b).orWith(nil), a.andWith(b));
        assertEquals(a.andWith(b).orWith(t), t);
        assertEquals(a.orWith(b).andWith(t), a.orWith(b));
        assertEquals(a.orWith(b).andWith(nil), nil);

        assertEquals(a.andWith(c.orWith(nil).not()), a.andWith(c.not()));
        assertEquals(nil.andWith(nil.orWith(b).not()), nil);
        assertEquals((a.andWith(c.orWith(nil).not())).orWith(nil.andWith(nil
                .orWith(b).not())), (a.andWith(c.not())));
        assertEquals(nil.orWith(c).andWith(b.orWith(nil).not()), c.andWith(b
                .not()));
