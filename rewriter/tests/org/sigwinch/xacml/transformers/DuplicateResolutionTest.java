...
public class DuplicateResolutionTest extends TestCase {
	...
	Tree tree = 
	    new FirstApplicableRule (new Scope (Permit.PERMIT,
	...
	assertEquals (expectedResult, 
		      tree.transform (new DuplicateRemover ()));
    }
	...
	Tree expectedResult =
	    new Error (Deny.DENY,
		       new OrPredicate (a, new OrPredicate (b, c)));
	assertEquals (expectedResult, 
		      tree.transform (new DuplicateRemover ()));
    }
	...
	Tree tree =
	    new Scope (new Scope (new Scope (Deny.DENY, a), b), c);
	Tree expectedResult =
	    new Scope (Deny.DENY, 
		       new AndPredicate (a, new AndPredicate (b, c)));
	assertEquals (expectedResult, 
		      tree.transform (new DuplicateRemover ()));
    }
	...
	Tree expectedResult = 
	    new Error (new Scope (Deny.DENY, new AndPredicate (a, b)), c);
	assertEquals (expectedResult, 
		      tree.transform (new DuplicateRemover ()));

	tree = new Scope (new Error (new Error (Deny.DENY, a), b), c);
	expectedResult = 
	    new Scope (new Error (Deny.DENY, new OrPredicate (a, b)), c);
	assertEquals (expectedResult, 
		      tree.transform (new DuplicateRemover ()));
    }
}
