...
public class InlineTest extends TestCase {
    ...
    public void testTrivialStuff() {
        out.start();
        new Triple(SimplePredicate.TRUE, SimplePredicate.FALSE, b).walk(out);
        out.end();
    ...
    public void testAnd() {
        out.start();
        new Triple(a.andWith(b), b.andWith(c), a.andWith(b).andWith(c))
                .walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
        assertEquals("one sig T0 extends Triple {} {\n" + "\tpermit = a & b\n"
    ...
    public void testOr() {
        out.start();
        new Triple(a.orWith(b), b.orWith(c), a.orWith(b).orWith(c)).walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
        assertEquals("one sig T0 extends Triple {} {\n" + "\tpermit = a + b\n"
                + "\tdeny = b + c\n" + "\terror = a + b + c\n" + "}\n", out
    ...
    public void testOrderOfOperations() {
        out.start();
        new Triple(a.orWith(b).andWith(c), a.andWith(b).orWith(c), a.andWith(b)
                .orWith(c).andWith(a)).walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
        assertEquals("one sig T0 extends Triple {} {\n"
                + "\tpermit = (a + b) & c\n" + "\tdeny = a & b + c\n"
    ...
    public void testSole() {
        out.start();
        new Triple(new SolePredicate(SimplePredicate.TRUE), new SolePredicate(
                SimplePredicate.TRUE), SimplePredicate.FALSE).walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
    ...
    public void testStupidFunction() {
        ...
        new FunctionCallPredicate("foo", new Predicate[] { a, b, c })
                .andWith(
                        new FunctionCallPredicate("foo", new Predicate[] { a,
                                b, c }))
                .orWith(new FunctionCallPredicate("bar", new Predicate[] { b }))
                .walk(out);
        out.end();
        assertEquals("one sig Functions {\n"
    ...
    public void testEqualsFunction() {
        ...
        new Triple(
                ...
                new FunctionCallPredicate(xp + "string-set-equals",
                        new Predicate[] { a, b }), SimplePredicate.TRUE)
                .walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
    ...
    public void testSizeFunction() {
        ...
        new Triple(new FunctionCallPredicate(xp + "string-bag-size",
                new Predicate[] { a }), new FunctionCallPredicate(xp
                + "string-bag-size", new Predicate[] { b }),
                SimplePredicate.FALSE).walk(out);
        out.end();
        assertEquals("one sig Functions {\n"
    ...
    public void testInclusionFunction() {
        ...
        new Triple(new FunctionCallPredicate(xp + "string-is-in",
                new Predicate[] { a, b }), new FunctionCallPredicate(xp
                + "string-is-in", new Predicate[] { b, c }),
                SimplePredicate.FALSE).walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
    ...
    public void testSetCreationFunction() {
        out.start();
        new Triple(new FunctionCallPredicate(xp + "string-bag",
                new Predicate[] { new FunctionCallPredicate(xp
                        + "string-bag-size", new Predicate[] { a }) }),
                SimplePredicate.TRUE, SimplePredicate.TRUE).walk(out);
        out.end();
        assertEquals("one sig Functions {\n"
                + "\texpr0 : E -> Integer // size of a\n" + "}\n", out
                .getFunctions());
    ...
    public void testSetManipulationFunctions() {
        ...
        new Triple(new FunctionCallPredicate(xp + "string-intersection",
                new Predicate[] { a, b }).andWith(new FunctionCallPredicate(xp
                + "integer-union", new Predicate[] { a, b })),
    ...
    public void testArithmeticFunctions() {
        out.start();
        new Triple(
                new FunctionCallPredicate(xp + "string-greater-than",
                        new Predicate[] { a, b })
                        .andWith(new FunctionCallPredicate(xp
                                + "string-greater-than-or-equal",
                                new Predicate[] { a, b })),
                new FunctionCallPredicate(FunctionVisitor.xacmlprefix
                        + "string-less-than", new Predicate[] { a, b }),
                new FunctionCallPredicate(FunctionVisitor.xacmlprefix
                        + "string-less-than-or-equal", new Predicate[] { a, b }))
                .walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
    ...
    public void testLogicalFunctions() {
        out.start();
        new Triple(new FunctionCallPredicate(xp + "and", new Predicate[] { a,
                b, c }), new FunctionCallPredicate(xp + "or", new Predicate[] {
                a, b, c }), new FunctionCallPredicate(xp + "not",
                new Predicate[] { a })).walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
        assertEquals("one sig T0 extends Triple {} {\n"
                + "\tpermit = a & b & c\n" + "\tdeny = a + b + c\n"
    ...
    public void testExistential() {
        out.start();
        new ExistentialPredicate(xp + "string-equal", b, a).walk(out);
        out.end();
        assertEquals("one sig Functions {\n}\n", out.getFunctions());
        assertEquals("sig S0 extends E {} { b in a }\n", out.getFacts());
        reset();
        out.start();
        new ExistentialPredicate(xp + "xpath-node-match", b, a).walk(out);
        out.end();
        assertEquals("one sig Functions {\n"
