...
public class CNFTest extends TestCase {
    ...
    public void testNot() {
        ...
        assertEquals(a, new Not(new Not(new Not(new Not(a)))).convertToCNF());
    }

    public void testAnd() {
        BooleanFormula formula = new And(a, b);
        assertEquals(formula, formula.convertToCNF());
        assertEquals(new And(new BooleanFormula[] { a, b, c }), new And(
                formula, c).convertToCNF());
        assertEquals(new And(new BooleanFormula[] { a, b, c }), new And(
                new Not(new Not(formula)), c).convertToCNF());
        assertEquals(new And(new BooleanFormula[] { a, b, c }), new And(
                formula, new Not(new Not(c))).convertToCNF());
        assertEquals(new And(new Or(a, c), new Or(b, c)), new Or(formula, c)
                .convertToCNF());
        assertEquals(new And(new Or(a, c), new Or(b, c)), new Or(c, formula)
                .convertToCNF());
    }
    ...
    public void testOr() {
        ...
        assertEquals(formula, formula.convertToCNF());
        assertEquals(new Or(new BooleanFormula[] { a, b, c }), new Or(formula,
                c).convertToCNF());
        assertEquals(new Or(new BooleanFormula[] { a, b, c }), new Or(new Not(
                new Not(formula)), c).convertToCNF());
        assertEquals(new Or(new BooleanFormula[] { a, b, c }), new Or(formula,
                new Not(new Not(c))).convertToCNF());
        assertEquals(new And(formula, b), new And(formula, b).convertToCNF());
        assertEquals(new And(formula, b), new And(b, formula).convertToCNF());
    }

    public void testDeMorgans() {
        assertEquals(new Or(a.negate(), b.negate()), new And(a, b).negate()
                .convertToCNF());
        assertEquals(new And(a.negate(), b.negate()), new Or(a, b).negate()
                .convertToCNF());
    }
