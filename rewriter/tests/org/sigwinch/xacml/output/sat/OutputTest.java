...
public class OutputTest extends TestCase {
    ...
    protected void setUp () {
        ...
        EnvironmentalPredicate baz = new EnvironmentalPredicate ("http://www.w3.org/2001/XMLSchema#string", "baz");
        stream = new StringWriter ();
        out = new AlloySatOutput (new PrintWriter (stream), 2.0);
        String xp = FunctionVisitor.xacmlprefix;
        Predicate.reset ();
        ConstantValuePredicate frobConstant = new ConstantValuePredicate ("frob", "quux");
        EnvironmentalPredicate frobSet = new EnvironmentalPredicate ("frob", "foo");
        stringIn = new FunctionCallPredicate (xp + "string-is-in",
                                            new Predicate [] {frobConstant, 
                                                              frobSet});
        ...
        (stringIn
         ...
         .andWith (new FunctionCallPredicate (xp + "string-equal", 
                                              new Predicate [] {new ConstantValuePredicate 
    ...
    public void testSmallOutput () {
        Triple t = new Triple (booleanConstant, SimplePredicate.TRUE, SimplePredicate.FALSE);
        out.preamble (t);
        out.write (t);
        out.postamble();
    ...
    public void testImplications () {
        Triple first = new Triple (booleanConstant, stringIn, SimplePredicate.FALSE);
        Triple second = new Triple (SimplePredicate.TRUE, booleanConstant, SimplePredicate.FALSE);
        out.preamble (null);
        out.write (first);
        out.write (second);
        out.postamble ();
