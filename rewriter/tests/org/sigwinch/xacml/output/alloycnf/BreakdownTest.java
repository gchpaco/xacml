...
public class BreakdownTest extends TestCase {
    ...
    public void testReferences() {
        new EnvironmentalPredicate("string", "foo").andWith(
                new ConstantValuePredicate("string", "foo")).walk(out);
        out.end();
        assertEquals("\tand0 : one Bool\n", out.getDecls());
        ...
        new FunctionCallPredicate("foo", new Predicate[] { a, b, c })
                .andWith(
                        new FunctionCallPredicate("foo", new Predicate[] { a,
                                b, c }))
                .orWith(new FunctionCallPredicate("foo", new Predicate[] { b }))
                .walk(out);
        out.end();
    ...
    public void testEqualsFunction() {
        ...
        new FunctionCallPredicate(xp + "string-set-equals", new Predicate[] {
                a, b }).walk(out);
        out.end();
    ...
    public void testSizeFunction() {
        new FunctionCallPredicate(xp + "string-bag-size", new Predicate[] { a })
                .walk(out);
        out.end();
        assertEquals("\texpr0 : one Integer // " + xp + "string-bag-size\n",
                out.getDecls());
        assertEquals("", out.getFacts());
    }

    public void testInclusionFunction() {
        new FunctionCallPredicate(FunctionVisitor.xacmlprefix + "string-is-in",
    ...
    public void testSetCreationFunction() {
        new FunctionCallPredicate(FunctionVisitor.xacmlprefix + "string-bag",
                new Predicate[] { new FunctionCallPredicate(
                        FunctionVisitor.xacmlprefix + "string-bag-size",
    ...
    public void testSetManipulationFunctions() {
        new FunctionCallPredicate(xp + "string-intersection", new Predicate[] {
                a, b }).walk(out);
    ...
    public void testArithmeticFunctions() {
        new FunctionCallPredicate(FunctionVisitor.xacmlprefix
                + "string-greater-than", new Predicate[] { a, b }).walk(out);
        new FunctionCallPredicate(FunctionVisitor.xacmlprefix
                + "string-greater-than-or-equal", new Predicate[] { a, b })
                .walk(out);
        new FunctionCallPredicate(FunctionVisitor.xacmlprefix
                + "string-less-than", new Predicate[] { a, b }).walk(out);
        new FunctionCallPredicate(FunctionVisitor.xacmlprefix
    ...
    public void testExistential() {
        ...
        new FunctionCallPredicate(xp + "not",
                new Predicate[] { new ExistentialPredicate(xp + "string-equal",
                        b, a) }).walk(out);
    ...
    public void testTriple() {
        ...
        new Triple(a.andWith(b), c, c).walk(out);
        out.end();
        assertEquals("Hash codes should be same", Integer.toString(a.andWith(b)
                .hashCode()), Integer.toString(a.andWith(b).hashCode()));
        assertEquals("\tand0 : one Bool\n", out.getDecls());
        assertEquals("\tX.and0 = And (a, b)\n", out.getFacts());
    }
}
