...
void set_homo_u32_vector_type(int argc, Scheme_Object **argv)
{
  if (argc != 1 )
    scheme_wrong_count("set_homo_u32_vector_type",
		       1, 1, argc, argv);
  ...
  while(listptr != scheme_null) {
	...
	(SCHEME_CDR(SCHEME_CDR(SCHEME_CAR(listptr))) != scheme_null)) {
      scheme_wrong_type("set_homo_u32_vector_type",
			"(listof (list/p string? integer?))",0,argc,argv);
    }
    
    if (strcmp(SCHEME_STR_VAL(SCHEME_CAR(SCHEME_CAR(listptr))),"u32") == 0) {
      scheme_get_int_val(SCHEME_CAR(SCHEME_CDR(SCHEME_CAR(listptr))),&tmp);
      homo_u32_vector_type = tmp; // assume cast succeeds.  GRR!
      return;
    }
    listptr = SCHEME_CDR(listptr);
  }
  scheme_signal_error("set_homo_u32_vector_type: type name u32 not found in initialization list");
}
