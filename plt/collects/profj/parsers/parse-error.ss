      ...
      (case state
         ...
         (if stmt-exp?
             ...
             (cond
               ...
               ((and (advanced?) (if-exp? tok))
                (parse-expression cur-tok (parse-expression cur-tok (getter) 'start getter #f stmt-exp?) 'if-exp-colon getter #f stmt-exp?))
                   ...
                   (let ((afterID (getter)))
                     (cond
                       ...
                       ((teaching-assignment-operator? (get-tok afterID))
                        (parse-expression next (parse-expression afterID (getter) 'start getter #f stmt-exp?)
        ...
        ((array-acc)
         (cond
           ...
           (else (parse-expression pre (parse-expression pre cur-tok 'start getter #f #f) 'c-bracket getter statement-ok? stmt-exp?))))
        ;Advanced
        ((c-bracket)
         (case kind
        ...
        ((cast-or-parened)
         (cond
           ...
           (else (parse-expression pre (parse-expression pre cur-tok 'start getter #f #f) 'c-paren getter statement-ok? stmt-exp?))))
        ;Intermediate
        ((cast)
         (cond
           ...
           ((O_PAREN)
            (let* ((next (getter))
              ...
              (cond
                ...
                (else (parse-expression next (parse-expression cur-tok next 'start getter #f stmt-exp?) 
